...
void calc_mag_sqr_meanss( 
   ...
   //float t_mag_sqr_sum = 0.0;
   if ( thread_index < (blockDim.x - mag_sqr_window_size)  ) {
      mag_sqr_means[thread_index] = 0.f;
      for( int w_index = 0; w_index < mag_sqr_window_size; ++w_index ) {
         __syncthreads();
         //t_mag_sqr_sum = t_mag_sqr_sum + mag_sqrs[thread_index + w_index];
         mag_sqr_means[thread_index] = mag_sqr_means[thread_index] + mag_sqrs[thread_index + w_index];
      }
      __syncthreads();
      //mag_sqr_means[thread_index] = t_mag_sqr_sum/(float)mag_sqr_window_size;
      mag_sqr_means[thread_index] = mag_sqr_means[thread_index]/(float)mag_sqr_window_size;
   }
}
...
void norm_autocorr_kernel( 
   ...
   s_samples[thread_index] = samples[global_index];
   __syncthreads();

   delay16<cufftComplex>( samples_d16, samples, num_samples );
   __syncthreads();
   s_samples_d16[thread_index] = samples_d16[global_index];
   __syncthreads();

   auto_correlations( s_conj_sqrs, s_samples_d16, s_samples, num_samples );
   __syncthreads();
   
   calc_conj_sqr_meanss( 
      s_conj_sqr_means, 
      s_conj_sqrs, 
   ...
   calc_conj_sqr_mean_magss( s_conj_sqr_mean_mags, s_conj_sqr_means, 
      num_conj_sqr_sums );
   __syncthreads();

   calc_mag_sqrss( s_mag_sqrs, s_samples, num_samples );
   __syncthreads();
   ...
   normalizes( s_norms, s_conj_sqr_mean_mags, s_mag_sqr_means, num_samples );
   __syncthreads();

   norms[global_index] = s_norms[thread_index];
}
